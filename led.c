...
uint16_t LedBri;									//buzzer Brightness
...
void Led_Init()
{
  ...
  CLK_EnableModuleClock(PWM0_MODULE);
	...
	LedDev.Feature.data3.maximum = 2048;
	LedDev.Feature.data3.value = 1;
...
// ----------------------------------------------------------------------------------------
//
void Led_Alerm(uint16_t freq, uint16_t duration, uint8_t level)
{
...
void Led_SingleCheck(uint16_t freq, uint16_t duration, uint8_t level)
{
...
void Led_Blink_Start()
{
	...
	switch (LedBlink)
	{
		case 0:
		{
			LedBri = LedDev.Feature.data2.value;
			...
			LedBlinkSize = 2;
			LedSingleCounter = 0;
			Led_Alerm(G7, LedToneStartTime, LedBri);
			break;
		}
		case 1:
		{
			...
			LedSingleCounter = 0;
			Led_Alerm(MusicScale[LedSingleCounter], MusicTime[LedSingleCounter], MusicLevel[LedSingleCounter]);
			break;
		}
	}
}
...
void Led_Blink_Check()
{
	if(LedStartFlag==1)
	{
		switch (LedBlink)
		{
			case 0:
			{
				if(LedSingleCounter == 0)	Led_SingleCheck(G7, LedToneStartTime, LedBri);
				else if(LedSingleCounter == 1)	Led_SingleCheck(G7, LedToneStopTime, 100);
				break;
			}
			case 1:
			{
				Led_SingleCheck(MusicScale[LedSingleCounter], MusicTime[LedSingleCounter], MusicLevel[LedSingleCounter]);
				break;
			}
			case 2:
			{
